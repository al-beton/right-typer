...
Use **Linear** for requirements, acceptance intent and product/scope decisions. Use **GitHub PRs** for implementation, reviews, fixes, QA, CI, stacks and delivery discussion. Prefix shared-account posts with the speaking role. Route the actual PR or inline-comment link directly to another task; do not wrap or mirror it in a Linear comment. Maintain normal issue status, ownership, dependencies and PR association as fields. Genuinely deferred work still gets a contextualized Linear issue defining its future scope.
...
At a handoff, interruption or restart, maintain compact recovery context in the existing PR description/thread (or this task before a PR exists):
...
On restart or handoff, reread the current `AGENTS.md`, that recovery context, the issue description and relevant PR threads, then verify live state before resuming. Record proposed specification/product changes on the assigned Linear issue. Keep implementation evidence and delivery milestones on the PR, linking them directly to affected tasks; no duplicate Linear checkpoint or status comment.
...
- Inspect the current UI and approved requirements. Propose coherent usable states, recovery, responsive behavior and accessibility with concrete visuals; record accepted design in Linear through the liaison. Supply and review visual acceptance evidence on the PR against the actual implementation and head, distinguishing proposals from verified behavior.
- Advise only; do not implement or merge unless separately assigned.

## Delivery manager

- Own the delivery queue, dependencies, assignment, CI/review/stack/merge supervision, PR recovery context and escalations. Maintain a clearly ordered, explicitly authorized ready queue in Linear with an unambiguous next issue and prerequisites. Do not silently enlarge scope or select unapproved product work.
...
- Publish a GitHub handoff requesting review, with the PR link and head SHA; record the head and next action on the PR, maintain ordinary Linear status/PR fields and route the public link directly to the reviewer. Start the next manager-authorized ready issue while review proceeds: stack dependent work on its unmerged parent branch; start independent work from current main. Never invent a next assignment.
...
- Check each fix and its evidence before resolving the corresponding thread. For an agreed non-blocking deferral, record the linked deferred disposition before resolving; do not describe it as fixed. Re-review changed or rebased heads. Merge only when acceptance, dependencies, current checks, conversations and repository rules permit; publish the result and merge SHA on the PR and route that link directly to the manager.

## Product tester

- Independently exercise current-head product journeys against authoritative acceptance, using isolated synthetic profiles and the [product test driver](docs/product-testing.md). Assess feedback, focus, layout and recovery; distinguish supplied observations from real-camera accuracy. Extend scenarios alongside implemented features without inventing absent-feature acceptance.
- Publish reproducible, role-prefixed findings and exact-head evidence in GitHub reviews; keep implementation recovery context on the PR and route its link directly to affected tasks. Builder fixes product defects, reviewer verifies fixes and owns merge. Implement only explicitly assigned test tooling; never self-merge or change product grading to make tests pass.
- Manager dispatches candidate, non-overlapping hourly and post-deploy runs. Coordinate shared helper/config files through the manager, keep current run coverage honest and stay quiet on unchanged failures. These team roles do not require individual contributors to run agents.

## Git and stack hygiene

1. Record parent/child PR links, intended bases and bottom-up merge order in the PRs. Review each incremental diff against its parent. Fix lower layers first; the builder restacks descendants, resolves conflicts preserving intent, updates PR bases/recovery context and refreshes validation and review.
