...
**This baseline has no measured exposure-error bound and is not yet accepted for release.** `uncertaintyMs: null` expresses that limitation. A callback/browser timestamp is not optical exposure; model turnaround and the ±500 ms search window do not establish an exposure bound. Native Safari/Studio Display and Chrome/Desk View staged and held-out correct/wrong-finger observations must evaluate whether the baseline is useful or needs a justified offset. A successful synthetic check does not meet that acceptance. Do not label the candidate hardware-verified or complete ALO-288.
