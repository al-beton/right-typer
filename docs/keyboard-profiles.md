...
The practice diagram shows a–z, comma, full stop and space at their physical positions. Labels show only exercised characters (French Shift+Comma is labelled full stop); the typing hint supplies required modifiers. Presets retain other positions for custom mapping. The unchanged English passage exercises lowercase letters, comma, full stop and space. Numbers and modifier fingers are not graded. ANSI/ISO is independent geometry metadata; unused positions, including the extra ISO key, are hidden from practice. Custom JSON can replace all geometry (including split arrangements).
