...
function render(){const press=data.presses[index];if(!press){el('key').textContent='No keypresses in this sample';return;}el('position').textContent=(index+1)+' / '+data.presses.length;el('key').textContent='Key '+(press.key===' '?'space':press.key)+' · press '+press.id+' · attempt '+press.attemptId;el('finger').value=data.labels[index].finger||data.labels[index].status;el('source').value=data.labels[index].source;frameIndex=data.frames.reduce((best,f,i)=>Math.abs(f.at-press.at)<Math.abs(data.frames[best].at-press.at)?i:best,0);renderFrame();}
