...
from publish import API, Publisher, SOURCE, approved, decorate, eligible_run, unpack, served

SHA = 'a' * 40
PR = {'number': 23, 'state': 'open', 'head': {'sha': SHA, 'repo': {'id': 1, 'full_name': SOURCE}}}
RUN = {'id': 99, 'workflow_id': 7, 'event': 'pull_request', 'conclusion': 'success',
       'head_sha': SHA, 'repository': {'full_name': SOURCE}, 'head_repository': {'id': 1},
...
class SecurityTests(unittest.TestCase):
    def test_valid_nested_assets(self):
        files = unpack(archive([('index.html', '<head></head>'), ('assets/model.wasm', b'\0asm')]))
    ...
    def test_reject_paths_links_duplicates_and_reserved_files(self):
        for path in ('../escape', '/absolute', 'a/../../b', 'a\\b', '.git/config',
                     '.github/workflows/x.yml', 'CNAME', 'preview.json', 'preview-banner.js', 'a\nb'):
    ...
    def test_run_identity(self):
        ...
        for field, value in [('workflow_id', 8), ('event', 'push'), ('conclusion', 'failure'),
                             ('head_sha', 'b' * 40), ('repository', {'full_name': 'attacker/repo'}),
                             ('head_repository', {'id': 2}), ('pull_requests', [])]:
    ...
    def test_manifest_is_not_enough_for_ready(self):
        import json
        files, manifest = decorate({'index.html': b'<head></head><body></body>'}, 23, SHA, 99)
        def fetch(url):
            name = url.split('/')[-1].split('?')[0]
            return files[name]
        with patch('publish.public_bytes', side_effect=fetch):
            self.assertTrue(served('https://review.example/pr-23/', manifest))
        with patch('publish.public_bytes', side_effect=[json.dumps(manifest).encode(), b'wrong']):
        ...
        self.assertLess(files['index.html'].index(b'preview-banner.js'), files['index.html'].index(b'</head>'))

    def test_separate_origin_required(self):
        for url in ('https://al-beton.github.io/another-repo/', 'http://review.example/'):
            with self.assertRaises(ValueError):
...
class LifecycleTests(unittest.TestCase):
    def publisher(self):
        p = Publisher(Mock(), Mock(), 'example/previews', 'https://example.github.io/previews/')
        p.comment = Mock()
        return p
    ...
    def test_changed_or_closed_head_fails_current(self):
        p = self.publisher()
        for pr in [dict(PR, state='closed'), dict(PR, head=dict(PR['head'], sha='b' * 40))]:
    ...
    def test_head_changes_during_upload_no_commit(self):
        p = self.publisher()
        artifact = {'name': f'preview-23-{SHA}', 'expired': False, 'id': 4, 'size_in_bytes': 100,
        ...
        p.current = Mock(return_value=False)
        p.commit = Mock()
    ...
    def test_update_preserves_other_preview_and_rechecks_after_publish(self):
        p = self.publisher()
        p.source.pages.return_value = [{'name': f'preview-23-{SHA}', 'expired': False, 'id': 4,
            'size_in_bytes': 100, 'workflow_run': {'id': 99, 'head_sha': SHA}}]
        p.source.call.return_value = archive([('index.html', '<head></head>')])
        p.target.call.return_value = {'sha': 'new-tree'}
        p.tree = Mock(return_value=('parent', [{'path': 'pr-24', 'sha': 'untouched'}, {'path': 'pr-23'}]))
