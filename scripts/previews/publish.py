    ...
    def call(self, path, method="GET", body=None, raw=False):
        ...
        request = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "right-typer-preview",
            },
        )
        if raw:
...
def approved(pr, api):
    ...
    for login, review in latest.items():
        if review["state"] == "APPROVED" and review["commit_id"] == pr["head"]["sha"]:
            permission = api.call(
                f"repos/{SOURCE}/collaborators/{urllib.parse.quote(login, safe='')}/permission"
            )
...
def eligible_run(run, pr, workflow_id):
    return (
        ...
        and run["head_repository"]
        and pr["head"]["repo"]
        and run["head_repository"]["id"] == pr["head"]["repo"]["id"]
        and (
            any(
                p["number"] == pr["number"] and p["head"]["sha"] == pr["head"]["sha"]
                for p in run["pull_requests"]
            )
            # GitHub omits pull_requests for fork runs. Bind to the live PR's
            # exact head repository, branch and SHA instead (all server data).
            or (not run["pull_requests"] and run["head_branch"] == pr["head"]["ref"])
        )
    )
...
def served(url, manifest):
    try:
        ...
        return all(
            hashlib.sha256(
                public_bytes(url + urllib.parse.quote(name) + "?verify=" + nonce)
            ).hexdigest()
            == digest
...
class Publisher:
    def __init__(self, source, target, repository, url):
        if (
            ...
            or repository.lower() == SOURCE.lower()
        ):
    ...
    def comment(self, number, body):
        ...
        existing = next(
            (
                c
                for c in self.source.pages(path)
        ...
        if existing:
            self.source.call(
                f"repos/{SOURCE}/issues/comments/{existing['id']}",
                "PATCH",
                {"body": body},
            )
        else:
    ...
    def publish(self, pr, run, workflow_id):
        ...
        found = [
            a
            for a in artifacts
            if a["name"] == f"preview-{number}-{sha}-{run['run_attempt']}"
            and not a["expired"]
        ]
        if len(found) != 1:
            self.comment(
                number,
                f"No usable artifact for current commit `{sha}`. Re-run Preview build, then Publish previews.",
            )
            return
        artifact = found[0]
        identity = artifact.get("workflow_run", {})
        if (
            identity.get("id") != run["id"]
            or identity.get("head_sha") != sha
        ...
        files, manifest = decorate(
            unpack(
                self.source.call(
                    f"repos/{SOURCE}/actions/artifacts/{int(artifact['id'])}/zip",
                    raw=True,
                )
            ),
            number,
        ...
        print(
            f"Committed PR #{number} at {sha}; verifying public file hashes", flush=True
        )
        for _ in range(40):
            if not self.current(number, sha):
                self.remove(number)
        ...
        self.comment(
            number,
            f"Deployment verification pending for `{sha}`. No ready link until every file matches. Rerun Publish previews to recover.",
        )

    def reconcile(self):
        workflow = self.source.call(
            f"repos/{SOURCE}/actions/workflows/preview-build.yml"
        )["id"]
        ...
        for pr in prs:
            ...
            if number not in allowed:
                self.comment(
                    number,
                    "Awaiting maintainer APPROVED review of the current head before fork publication. Re-run Publish previews after approval.",
                )
                continue
            try:
                runs = self.source.pages(
                    f"repos/{SOURCE}/actions/workflows/{workflow}/runs?event=pull_request&head_sha={pr['head']['sha']}",
                    "workflow_runs",
                )
                ...
                if candidates:
                    self.publish(
                        ...
                        max(
                            candidates,
                            key=lambda r: (r["run_number"], r["run_attempt"]),
                        ),
                        workflow,
                    )
                else:
                    self.comment(
                        number,
                        f"Waiting for a successful Preview build of current commit `{pr['head']['sha']}`.",
                    )
            except Exception as error:  # noqa: BLE001 - isolate hostile PRs and redact signed URLs
                failures.append(number)
                self.comment(
                    number,
                    f"Preview failed validation/publication ({type(error).__name__}). Inspect the workflow and rerun; no Ready link is advertised.",
                )
                print(f"PR #{number}: {type(error).__name__}", file=sys.stderr)
        if failures:
            raise RuntimeError(
                "One or more previews failed; other PRs were still reconciled"
            )
...
if __name__ == "__main__":
    try:
        Publisher(
            API(os.environ["SOURCE_TOKEN"]),
            API(os.environ["PREVIEW_DEPLOY_TOKEN"]),
            os.environ["PREVIEW_REPOSITORY"],
            os.environ["PREVIEW_BASE_URL"],
        ).reconcile()
    except Exception as error:  # noqa: BLE001 - do not expose signed URLs in tracebacks
        print(
            f"Preview reconciliation failed ({type(error).__name__}); inspect configuration and rerun.",
            file=sys.stderr,
        )
        sys.exit(1)
