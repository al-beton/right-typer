...
Calibration, camera selection, rotation, whether you started practice, and the last ten completed results are saved in this browser’s local storage. Refresh reopens the selected camera and starts a fresh passage automatically after starting, provided its saved positions match the camera and image dimensions. Pause offers Resume practice at the word entry. Settings → Camera → Edit key positions opens mapping adjustments. Both keep practice paused across refreshes. Resume keeps completed words and starts the current word afresh. Practise again starts a new passage directly when setup is valid. Inspect the overlaid dots and adjust them if the camera moved; a different camera requires remapping. An interrupted passage stays only in memory. **Practice & history** contains completed result details and their counting rules. **Local data & about → Reset local data** clears calibration, preferences and history after a second confirming click.
