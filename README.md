...
Open the printed localhost URL in current Chrome on your external display. The page requests camera access immediately; allow it in Chrome. The camera button remains available to retry or restart. Tilt the lid until both hands and the letter area/spacebar fit, and map the key centers. The typing area, finger map and camera stay on one page. Go unlocks once all 30 required positions are valid. The optional test field shows observed fingers; swap left/right labels if needed. No hand-visibility or confirmation checkbox gate is required.
