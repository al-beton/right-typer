...
import { syntheticCamera, openSettings, setup, press, word } from './helpers';
...
test('window input attributes correct and wrong fingers, retains unknowns, stops and requires a fresh map', async ({
  page,
}) => {
  await displayInput(page);
  expect(await page.evaluate(() => window.__displayRequests)).toBe(0);
  await page.locator('#share-window').click();
  await expect(page.locator('#camera-badge')).toContainText('hands detected · timing estimated');
  expect(await page.evaluate(() => window.__displayOptions?.audio)).toBe(false);
  await expect(page.locator('#start-camera')).toHaveText('Use selected camera');
  await map(page);
  ...
  await page.screenshot({ path: 'test-results/window-practice-synthetic.png', fullPage: true });
...
test('real bundled model consumes synthetic display pixels with local requests and honest timing', async ({
  ...
  page.on('request', (r) => {
    if (!r.url().startsWith(new URL(baseURL!).origin + '/') && !r.url().startsWith('data:'))
      external.push(r.url());
  });
  await displayInput(page, true);
  await page.locator('#share-window').click();
  await expect(page.locator('#camera-badge')).toContainText('0 hands detected · timing estimated', {
    timeout: 30000,
  });
...
test('window practice preserves the saved webcam map and webcam reconnect reuses it', async ({
  ...
  expect(
    await page.evaluate(() => JSON.parse(localStorage.getItem('right-typer.v1')!).calibration),
  ).toMatchObject({ ...saved, savedAt: expect.any(Number) });
  await openSettings(page);
